...
<div class="row">
  <div class="main">
    ...
    ?>
    <h2>Rent available</h2>
    <table table border="1" style="font-size:16px;font-family:serif;" cellpadding="4">
        <thead>
            <tr>
            ...
            <?php
                // FILL TABLE WITH DATA ON CLICK
                  $sql = "SELECT * FROM `Apartment_units` A WHERE (A.`unit_name`,A.`company_ID`) NOT IN (SELECT `unit_name`,`company_ID` From L_available UNION ALL SELECT `unit_name`,`company_ID` from C_available)";
   ...
   <br>
   <h3>SQL code</h3>
   <p>SELECT * FROM `Apartment_units` A</p>
   <p>WHERE (A.`unit_name`,A.`company_ID`)</p>
   <p>NOT IN (SELECT `unit_name`,`company_ID`</p>
   <p>From L_available UNION ALL SELECT `unit_name`,`company_ID` from</p>
   <p>C_available)</p>
